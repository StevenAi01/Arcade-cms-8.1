                            ...
                            $seo_name = seo_friendly_url($ec_name);
                             ...
                             if (isset($_FILES['__game_image']['tmp_name'])) {
                                  ...
                                  if ($_FILES['__game_image']['size'] > 1024) {
                                        ...
                                        $GameMonetizeConnect->query("UPDATE ".CATEGORIES." SET category_pilot='{$seo_name}',name='{$ec_name}',image='{$addgame_mediaurl}',footer_description='{$ec_footer_description}',show_home='{$ec_show_home}' WHERE id='{$ec_id}'");
                                                
                                    } else { $data['error_message'] = $lang['error_image_size']; }
                                    } else { $data['error_message'] = $lang['message_select_img_files']; }
                            }
                            else {
                               $GameMonetizeConnect->query("UPDATE ".CATEGORIES." SET category_pilot='{$seo_name}',name='{$ec_name}',footer_description='{$ec_footer_description}',show_home='{$ec_show_home}' WHERE id='{$ec_id}'");
                            }
