...
if ($_GET['p'] != 'login') {
	if (
		...
		if ($config['site_theme'] == 'poki-like' || $config['site_theme'] == 'y8-like') {
				...
				if ($sidebar['type'] != "separator" && $sidebar['type'] != "search") {
					if ($sidebar['type'] == "category") {
						...
						if ($categoryData !== null) {
							...
							$themeData['category_name'] = $categoryData['name'];
							$themeData['category_image'] = $categoryData['image'];
	
							$themeData['category_number'] = $numbergames;
							$themeData['category_url'] = siteUrl() . '/category/'	. slugify($categoryData['name']);
							$ct_r .= \GameMonetize\UI::view('category/categories-list-2');
						}
					}
					...
					if ($sidebar['type'] == "tags") {
						...
						if ($tagsData !== null) {
							...
							$themeData['tag_name'] = $tagsData['name'];
							
							$baseTagImagePath = '/tag-img/' . slugify($tagsData['name']);
							$formats = ['.png', '.webp'];
							$defaultTagImagePath = siteUrl() . '/templates/crazygames-like/image/tag.png';

							$themeData['tag_image'] = $defaultTagImagePath; // Default value

							foreach ($formats as $format) {
								if (file_exists($_SERVER['DOCUMENT_ROOT'] . $baseTagImagePath . $format)) {
									$themeData['tag_image'] = siteUrl() . $baseTagImagePath . $format;
									break;
								}
							}
	
							$themeData['tag_url'] = siteUrl() . '/tag/'	. slugify($tagsData['name']);
							$tag_r .= \GameMonetize\UI::view('tags/tags-list-home');
						}
					}
				}
		...
		if ($settingData['is_sidebar_enabled'] && $config['site_theme'] != 'crazygames-like') {
			$themeData['header'] = "";
